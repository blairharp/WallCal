...
export function EventDetail() {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative w-full max-w-lg bg-slate-900 border border-slate-700 rounded-t-2xl p-6 pb-10 shadow-2xl animate-slide-up"
      ...
      >
        ...
        <button
          onClick={() => setSelectedEvent(null)}
          className="absolute top-4 right-4 p-1 rounded-full text-slate-500 hover:text-slate-300 hover:bg-slate-800 transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="flex items-start gap-3 mt-2 mb-4">
          ...
          />
          <h2 className="text-xl font-semibold text-slate-50 leading-snug pr-6">
            {selectedEvent.title}
          </h2>
        </div>
